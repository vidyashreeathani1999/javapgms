public class LuceneSearchUtil {
    ...
    private static final String DB_URL = "jdbc:sqlite:C:/Users/2114779/IdeaProjects/GWErrorAssist/res/gw_error_assist.db";
    private static final String INDEX_DIR = "C:/Users/2114779/IdeaProjects/GWErrorAssist/res/";
    private static final String SEARCH_SQL = "SELECT id, Description FROM TABLE1";//"SELECT ID, Description  FROM project1";
    ...
    public void addDocument(int id, String content) throws IOException {
        if(content == null || content.trim().isEmpty() || id <=0)  {
            System.err.println("Skipper document with empty");
            return;
        }
    ...
    public List<String> search(String queryString) throws Exception {
        ...
        for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
            ...
            results.add(doc.get("INDEX_FIELD_NAME"));//"ID"));//INDEX_FIELD_NAME));
        }
        return results;
    }
...
public class SqliteUtil {
    ...
    static final String DB_URL = "jdbc:sqlite:C:/Users/2114779/IdeaProjects/GWErrorAssist/res/gw_error_assist.db";
    ...
    public static void createNewDatabase()
    {
        ...
        try {
            ...
            String url = DB_URL; //Override the url with DB_URL when running to create a new database
...
public class AppConstants {
    ...
    public static final String JDBC_URL = "jdbc:sqlite:C:/Users/2114779/IdeaProjects/GWErrorAssist/res/gw_error_assist.db";
            //"jdbc:sqlite::res:gw_error_assist.db";
...
public class ErrorAssistApplication extends Application {
    ...
    private List<ErrorAssistTO> performSearch(String query) throws SQLException {
        List<ErrorAssistTO> resultList = new ArrayList<>();

        try (Connection conn = DriverManager.getConnection(AppConstants.JDBC_URL);
             PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM TABLE1 WHERE Description LIKE ? or Cause LIKE ?")) {
            pstmt.setString(1, "%" + query + "%");
            pstmt.setString(2,"%"+ query +"%");
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ErrorAssistTO errorAssistTO = new ErrorAssistTO();
                    errorAssistTO.rowID = Integer.toString(rs.getInt("id"));
                    errorAssistTO.applicationName = rs.getString("Application");
                    errorAssistTO.versions = rs.getString("Applies_To");
                    errorAssistTO.errorCode = rs.getString("Error_Code");
                    errorAssistTO.description = rs.getString("Description");
                    errorAssistTO.cause = rs.getString("Cause");
                    errorAssistTO.solution = rs.getString("Solution");
                    errorAssistTO.otherComments = rs.getString("Other");
                    errorAssistTO.source = rs.getString("Source");
                    errorAssistTO.lastUpdated = rs.getString("Last_Updated");

                    resultList.add(errorAssistTO);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
...
public class DBManager {
    ...
    private static final String SEARCH_SQL_HQ = "SELECT * FROM TABLE1 WHERE id IN (";
...
ResultSet resultSet = null;
        try {
            ...
            try (PreparedStatement statement = connection.prepareStatement(thisSearchQuery)) {
                int validIdCount = 0;
                for (int i = 0; i < rowIDList.size(); i++) {
                    String thisID = rowIDList.get(i);
                    try {
                        //Integer thisID = Integer.valueOf(rowIDList.get(i));
                        Integer id = Integer.valueOf(thisID);
                        //  statement.setInt(i + 1, id);
                        statement.setInt(validIdCount + 1, id);
                        validIdCount++;
                    } catch (NumberFormatException e) {
                        System.err.println("Invalid ID: " + thisID);
                    }
                }
                if (validIdCount > 0) {
                    resultSet = statement.executeQuery();
                } else {
                    System.err.println("No valid id to query.");
                }
...
public class ErrorAssistTranformer {
    ...
    public static ArrayList<ErrorAssistTO> transformResultToEntity(ResultSet resultSet) throws SQLException
    {
        ...
        if (resultSet !=null) {
            ...
            try {
                while (resultSet.next()) {
                    ...
                    thisErrorAssistTO.rowID = Integer.toString(resultSet.getInt("id"));
...
public class ErrorAssistService {
    ...
    private static List<ErrorAssistTO> searchAndRetrieve(String searchStr) {
        ...
        List<ErrorAssistTO> errorAssistTOList = null;
        ...
        try {
           ...
           // ArrayList<ErrorAssistTO> errorAssitTOList = ErrorAssistTranformer.transformResultToEntity(resultSet);
            resultList = ErrorAssistTranformer.transformResultToEntity(resultSet);

            return  ErrorAssistTranformer.transformResultToEntity(resultSet);
           /* if (errorAssitTOList != null) {
                resultList.addAll(errorAssitTOList);
            }
